...
## 1.26.0 - 2026-08-21
- **CHG:** Skalares `apiurl` durch das Array-Feld `apiurls` ersetzt (`typ: "array"`, Eintrag `bevoelkerung`). Neuer Standard portfolioweit; `apiurl` entfällt. `app.js` liest die Datenquelle jetzt über `getOdasApiUrl(configdata, "bevoelkerung")`.
...
## 1.24.0 - 2026-08-20
- FIX: Generierte IDs (`odas-filter-*`, `odas-chart`) tragen jetzt durchgängig die Instanzkennung (F-71)
- FIX: Zeilen mit ungültigem `WERT` werden jetzt gezählt und als Hinweis angezeigt statt kommentarlos verworfen (F-73)
